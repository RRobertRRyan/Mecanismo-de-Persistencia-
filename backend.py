"""
The Sort Engine — Backend Flask
Algoritmos de ordenação e busca com dados do Banco Mundial
+ Camada de persistência: CSV (texto) e Pickle (binário)
"""
...
# ══════════════════════════════════════════════
#  ALGORITMOS DE ORDENAÇÃO (com passo a passo)
# ══════════════════════════════════════════════
    ...
    for i in range(n):
        ...
        for j in range(i+1, n):
            ...
            if a[j]["value"] < a[min_idx]["value"]:
                min_idx = j
            steps.append({
                "array": [x["name"] for x in a], "values": [x["value"] for x in a],
    ...
    def _merge(lst, lo, mid, hi):
        ...
        while i < len(left) and j < len(right):
            ...
            if left[i]["value"] <= right[j]["value"]:
                ...
                swaps_box[0] += 1
            k += 1
            steps.append({"array": [x["name"] for x in lst],
...
# Fallback de países reais, ordenados por PIB (US$ bilhões, 2022)
FALLBACK = [
    {"name":"EUA","value":25463.0},
    {"name":"China","value":17963.2},
    {"name":"Japão","value":4231.1},
    {"name":"Alemanha","value":4072.2},
    {"name":"Índia","value":3385.1},
    {"name":"Reino Unido","value":3070.7},
    {"name":"França","value":2782.9},
    {"name":"Canadá","value":2139.8},
    {"name":"Brasil","value":2081.2},
    {"name":"Itália","value":2010.4},
    {"name":"Austrália","value":1693.0},
    {"name":"Coreia do Sul","value":1665.2},
    {"name":"Espanha","value":1418.3},
    {"name":"México","value":1322.8},
    {"name":"Indonésia","value":1319.1},
    {"name":"Países Baixos","value":1008.0},
    {"name":"Suíça","value":807.7},
    {"name":"Turquia","value":905.9},
    {"name":"Polônia","value":688.2},
    {"name":"Suécia","value":585.9},
    {"name":"Irlanda","value":529.3},
    {"name":"Israel","value":527.5},
    {"name":"Argentina","value":632.8},
    {"name":"Bélgica","value":579.2},
    {"name":"Noruega","value":579.2},
    {"name":"Emirados Árabes","value":499.2},
    {"name":"Nigéria","value":477.4},
    {"name":"Áustria","value":471.4},
    {"name":"Bangladesh","value":460.2},
    {"name":"Singapura","value":466.8},
    {"name":"Tailândia","value":495.4},
    {"name":"Malásia","value":407.0},
    {"name":"Dinamarca","value":395.1},
    {"name":"Egito","value":387.1},
    {"name":"Vietnã","value":362.6},
    {"name":"Colômbia","value":343.7},
    {"name":"Romênia","value":301.4},
    {"name":"Chile","value":301.0},
    {"name":"Republica Tcheca","value":296.3},
    {"name":"Iraque","value":264.2},
    {"name":"Finlândia","value":267.3},
    {"name":"Nova Zelândia","value":246.7},
    {"name":"Peru","value":242.6},
    {"name":"Portugal","value":237.7},
    {"name":"Cazaquistão","value":220.6},
    {"name":"Grécia","value":218.2},
    {"name":"Hungria","value":188.5},
    {"name":"Ucrânia","value":160.5},
    {"name":"Etiópia","value":126.8},
    {"name":"Angola","value":124.5},
]


# Palavras que identificam agregados/regiões (não países reais)
_REGION_WORDS = {
    "income", "world", "ibrd", "ida ", "oecd", "euro area",
    "fragile", "dividend", "blend", "total", "caribbean",
    "sub-saharan", "latin", "north africa", "south asia",
...
def fetch_world_bank(indicator, divisor, size):
    """
    Busca dados do Banco Mundial filtrando apenas países reais.
    A API /country/all inclui regiões e grupos econômicos — filtramos
    pelo campo countryiso3code (3 letras = país real, 2 = agregado).
    Se a rede falhar, usa FALLBACK com países reais pré-definidos.
    """
    ...
    try:
        ...
        for entry in raw[1] or []:
            ...
            # countryiso3code com exatamente 3 letras = país real
            # (regiões/agregados têm 2 letras ou string vazia)
            if not iso3 or len(iso3) != 3:
                continue
            # Filtro extra por palavras-chave no nome
            if _is_region(name):
                continue

            items.append({
                "name":  name,
                "value": round(entry["value"] / divisor, 2),
            })

        if not items:
            raise ValueError("Sem países após filtro")

        items.sort(key=lambda x: x["value"], reverse=True)
        return items[:size]

    except Exception:
        # Fallback com países reais — já ordenados por PIB decrescente
        return [d.copy() for d in FALLBACK[:size]]

# ══════════════════════════════════════════════
#  PERSISTÊNCIA — CSV e Pickle
# ══════════════════════════════════════════════
...
@app.route("/api/dados")
def ep_dados():
    """Busca da API do Banco Mundial (com fallback offline)."""
...
def ep_sort():
    ...
    if not fn:
        return jsonify({"ok": False, "erro": "algoritmo inválido"}), 400
    ...
    return jsonify({
        "ok": True, "fonte": fonte,
        "sorted":  sorted_arr,
        "steps":   steps[:500],   # limita para não estourar JSON
...
def ep_busca():
    ...
    tipo    = request.args.get("tipo", "substring")   # exact | substring
    offline = request.args.get("offline", "false").lower() == "true"
    fmt     = request.args.get("fmt", "csv")
    size    = min(int(request.args.get("size", 16)), 50)
    ...
    if offline:
        if fmt == "pkl" and os.path.exists(FILES["pkl"]):
            ...
            return jsonify({"ok": False, "erro": "Salve primeiro."}), 404
        dados = dados[:size]
    else:
        indicator, divisor, _ = WB_SOURCES.get(source, WB_SOURCES["world_bank_gdp"])
        dados = fetch_world_bank(indicator, divisor, size)
    ...
    def busca_binaria(arr, q):
        ...
        while lo <= hi:
            ...
            v = s[mid]["value"]
            if abs(v - alvo_f) < 0.01:
                encontrado = {"index": mid, "item": s[mid]}; break
            elif v < alvo_f: lo = mid + 1
            else:            hi = mid - 1
        return passos, [encontrado] if encontrado else []

    t0 = time.perf_counter()
    passos_lin,  res_lin  = busca_linear(dados, alvo, tipo)
    passos_bin,  res_bin  = busca_binaria(dados, alvo)
    elapsed = round((time.perf_counter() - t0) * 1000, 3)

    return jsonify({
        "ok": True,
        "linear":  {"passos": passos_lin,  "encontrados": res_lin},
        "binaria": {"passos": passos_bin,  "encontrados": res_bin},
        "time_ms": elapsed,
        "total":   len(dados),
    })
...
def ep_comparar():
    ...
    if offline:
        if fmt == "pkl" and os.path.exists(FILES["pkl"]):
            dados_base = carregar_pickle()
        elif os.path.exists(FILES["csv"]):
            dados_base = carregar_csv()
        else:
            return jsonify({"ok": False, "erro": "Salve primeiro."}), 404
    ...
    for algo in algos:
        ...
        elapsed = round((time.perf_counter() - t0) * 1000, 2)
        resultado[algo] = {"comparisons": comps, "swaps": swaps, "time_ms": elapsed}
...
def ep_salvar():
    ...
    for fmt, fn in [("csv", salvar_csv), ("pkl", salvar_pickle)]:
        ...
        fn(dados)
        metricas[fmt] = {
            "tempo_ms":  round((time.perf_counter() - t0) * 1000, 3),
...
def ep_offline():
    ...
    if not os.path.exists(caminho):
        return jsonify({"ok": False, "erro": f"'{caminho}' não existe. Salve primeiro."}), 404
...
# ── Painel comparativo de formatos ─────────────
...
def ep_painel():
    ...
    for fmt, caminho in FILES.items():
        ...
        resultado[fmt] = {
            "disponivel":    True,
            "tamanho_kb":    tamanho_kb(caminho),
            "tempo_leitura_ms": round((time.perf_counter() - t0) * 1000, 3),
        }
    return jsonify(resultado)


# ── Inspeção texto vs binário ──────────────────
...
if __name__ == "__main__":
    print("✅  The Sort Engine rodando em http://localhost:5000")
